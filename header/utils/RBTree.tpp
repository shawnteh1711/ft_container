...
/*   Updated: 2023/02/01 22:45:22 by steh             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ft::RBTree<T, KeyofValue, Compare, Alloc>::insert(const T& value)
{
	...
	while (current != _TNULL)
	{
		...
		if (_comp(key, _keyofvalue(current->data)))
			...
			return (ft::make_pair(iterator(current, _TNULL), false)); /// so need pass, tnull
	}
	...
	{
		new_node->color = black;
		return (ft::make_pair(iterator(new_node, _TNULL), true));
	}
	if (new_node->parent->parent == nullptr || new_node->parent->parent == _TNULL)
		return (ft::make_pair(iterator(new_node, _TNULL), false));
	algo<Node, T, KeyofValue, Compare, Alloc>::insert_fix(new_node);
	return (ft::make_pair(iterator(new_node, _TNULL), true));
}
...
void ft::RBTree<T, KeyofValue, Compare, Alloc>::print_tree()
{
	if (_root)
	{
		algo<Node, T, KeyofValue, Compare, Alloc>::print_helper(this->_root, "", true);
		// algo<Node, T, KeyofValue, Compare, Alloc>::print_helper(this->_root, "", true);
	}
}
...
{
	return (const_iterator(minimum(_root), _TNULL));
}
